...
namespace op {
// 输入: 向量 in1 = w1 * x (immediate_dim) 和向量 in2 = w2 * x (immediate_dim)
// 输出: 向量 out (immediate_dim)
// 计算公式: 其中 @ 表示逐位相乘
...
public:
    explicit SwiGLULayer(base::DeviceType device_type, int32_t immediate_dim);
...
private:
    int32_t immediate_dim_ = 0; // 隐藏层大小: 当输入张量大小为 d 时，隐藏层大小为 (2/3) * 4d
};
}  // namespace op
