...
//     }
//     for (auto& o_proj_layer : o_proj_layers_) {
//         o_proj_layer->set_cuda_config(cuda_config);
//         o_proj_layer->to_cuda();
//     }
//     for (auto& fused_add_rmsnorm_layer : fused_add_rmsnorm_layers_) {
//         fused_add_rmsnorm_layer->set_cuda_config(cuda_config);
//         fused_add_rmsnorm_layer->to_cuda();
//     }
...
//     }
//     for (auto& fused_down_add_layer_ : fused_down_add_layers_) {
...
//     }
//     if (qwen3_fused_layers_->o_proj_layers_.size() != config_->layer_num) {
//         return base::error::internal_error("Create the o-proj layers for the qwen3 model failed!");
//     }
//     if (qwen3_fused_layers_->fused_add_rmsnorm_layers_.size() != config_->layer_num) {
//         return base::error::internal_error("Create the fused-add-rmsnorm layers for the qwen3 model failed!");
//     }
...
//     }
//     if (qwen3_fused_layers_->fused_down_add_layers_.size() != config_->layer_num) {
...
//             !qwen3_fused_layers_->o_proj_layers_[i] ||
//             !qwen3_fused_layers_->fused_add_rmsnorm_layers_[i] ||
//             !qwen3_fused_layers_->fused_gate_up_swiglu_layers_[i] ||
//             !qwen3_fused_layers_->fused_down_add_layers_[i]) {
...
//     qwen3_fused_layers_->flashdecoding_gqa_layer_ = std::make_unique<op::MultiHeadAttention>(device_type_, kv_dim, kv_mul, head_num, head_dim, max_seq_len);
// }
...
//     for (int32_t i = 0; i < layer_num; ++i) {
//         auto w_qkv = std::make_unique<op::QKVMatmulLayer>(device_type_, dim, kv_dim, dim + 2 * kv_dim, hidden_dim);
...
//     for (int32_t i = 0; i < layer_num; ++i) {
//         auto qk_norm = std::make_unique<op::QKNormRoPELaryer>(device_type_, 2 * head_dim);
...
//     // 5. O_Proj (w_o) : [layers, hidden_dim, dim]
//     for (int32_t i = 0; i < layer_num; ++i) {
//         auto w_o = std::make_unique<op::MatmulLayer>(device_type_, hidden_dim, dim);
//         w_o->set_weight(0, {hidden_dim, dim}, raw_model_data_->weight_ptr(offset), base::DeviceType::DeviceCPU);
//         qwen3_fused_layers_->o_proj_layers_.push_back(std::move(w_o));
//         offset += hidden_dim * dim;
//     }

//     // 6. Fused_Add_RMSNorm (rmsnorm) : [layers, hidden_dim]
//     for (int32_t i = 0; i < layer_num; ++i) {
//         auto add_rmsnorm = std::make_unique<op::RMSNormLaryer>(device_type_, hidden_dim, true); // fuse_add = true
//         add_rmsnorm->set_weight(0, {hidden_dim}, raw_model_data_->weight_ptr(offset), base::DeviceType::DeviceCPU);
//         qwen3_fused_layers_->fused_add_rmsnorm_layers_.push_back(std::move(add_rmsnorm));
//         offset += hidden_dim;
//     }

//     // 7. Fused_Gate_Up_Swiglu (w_gate + w_up) : [layers, 2 * immediate_dim, hidden_dim]
//     for (int32_t i = 0; i < layer_num; ++i) {
//         auto w_gate_up = std::make_unique<op::GateUpSwigluLayer>(device_type_, 2 * immediate_dim, hidden_dim);
...
//     for (int32_t i = 0; i < layer_num; ++i) {
//         auto w_down = std::make_unique<op::MatmulLayer>(device_type_, hidden_dim, immediate_dim, true); // fuse_add = true
//         w_down->set_weight(0, {hidden_dim, immediate_dim}, raw_model_data_->weight_ptr(offset), base::DeviceType::DeviceCPU);
//         qwen3_fused_layers_->fused_down_add_layers_.push_back(std::move(w_down));
...
//     qwen3_fused_layers_->lm_head_layer_ = std::make_unique<op::MatmulLayer>(device_type_, vocab_size, hidden_dim);
...
// void Qwen3FusedModel::attention_rmsnorm(int32_t layer_id, const tensor::Tensor& input) const {
//     const auto& mha_rmsnorm = qwen3_fused_layers_->pre_rmsnorm_layers_.at(layer_id);
//     STATUS_CHECK(mha_rmsnorm->forward(input, get_buffer(ModelBufferType::MHAPreRMSNorm)));
// }

// void Qwen3FusedModel::attention_qkv_rope(int32_t layer_id, const tensor::Tensor& token_pos) const {
//     int32_t dim = config_->dim;
//     int32_t kv_dim = config_->kv_dim;
//     int32_t head_dim = config_->head_dim;
//     int32_t head_num = config_->head_num;
...
//     const auto& wq_layer = qwen3_fused_layers_->wq_layers_.at(layer_id);
//     const auto& wk_layer = qwen3_fused_layers_->wk_layers_.at(layer_id);
//     const auto& wv_layer = qwen3_fused_layers_->wv_layers_.at(layer_id);

//     const auto& q_norm = qwen3_fused_layers_->rmsnorm_layers_.at(layer_id + 2 * layer_num + 1);
//     const auto& k_norm = qwen3_fused_layers_->rmsnorm_layers_.at(layer_id + 3 * layer_num + 1);
...
//     const tensor::Tensor& sin_cache = get_buffer(ModelBufferType::SinCache);
//     const tensor::Tensor& cos_cache = get_buffer(ModelBufferType::CosCache);
//     STATUS_CHECK(qwen3_fused_layers_->rope_layer_->forward(query, key, token_pos, sin_cache, cos_cache, tensor::Tensor()));
// }

// void Qwen3FusedModel::attention_mha(int32_t layer_id, const tensor::Tensor& token_pos) const {
...
//     op::MultiHeadAttention* mha_layer_ptr = dynamic_cast<op::MultiHeadAttention*>(qwen3_fused_layers_->mha_layer_.get());
...
//     STATUS_CHECK(qwen3_fused_layers_->mha_layer_->forward(query, key_cache, value_cache, kv_split_output, output));

//     // 4. 输出投影: 计算出的结果再经过一个 wo (Output Weight) 线性层
//     const auto& wo_layer = qwen3_fused_layers_->wo_layers_.at(layer_id);
//     STATUS_CHECK(wo_layer->forward(output, get_buffer(ModelBufferType::AttentionOuput)));
// }

// void Qwen3FusedModel::feed_forward(int32_t layer_id, const tensor::Tensor& input) const {
//     // 1. 残差连接 (Residual Add): 把刚才 Attention 的结果加回到原始 input 上
//     STATUS_CHECK(qwen3_fused_layers_->add_layer_->forward(input, get_buffer(ModelBufferType::AttentionOuput), input));

//     // 2. FFN 归一化: 对加完的数据再做一次 RMSNorm
//     const auto& ffn_rmsnorm = qwen3_fused_layers_->rmsnorm_layers_.at(config_->layer_num + layer_id);
...
//     const auto& w1_layer = qwen3_fused_layers_->w1_layers_.at(layer_id);
//     const auto& w2_layer = qwen3_fused_layers_->w2_layers_.at(layer_id);
//     const auto& w3_layer = qwen3_fused_layers_->w3_layers_.at(layer_id);
...
//     STATUS_CHECK(qwen3_fused_layers_->swiglu_layer_->forward(w1_output, w3_output, w1_output));
//     STATUS_CHECK(w2_layer->forward(w1_output, w2_output));

//     // 4. 残差连接 (Residual Add): 再次把 FFN 的结果加回 input
//     STATUS_CHECK(qwen3_fused_layers_->add_layer_->forward(input, w2_output, input));
// }

// void Qwen3FusedModel::cls_logits(const tensor::Tensor& input) const {
//     // 1. 最后再做一次 Final RMSNorm
//     const auto& final_rmsnorm = qwen3_fused_layers_->rmsnorm_layers_.at(2 * config_->layer_num);
...
//     STATUS_CHECK(qwen3_fused_layers_->cls_layer_->forward(input, logits));
// }
