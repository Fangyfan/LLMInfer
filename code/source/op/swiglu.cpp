...
namespace op {
SwiGLULayer::SwiGLULayer(base::DeviceType device_type, int32_t immediate_dim)
: Layer(device_type, LayerType::LayerSwiGLU, "SwiGLU"), immediate_dim_(immediate_dim) {
    reset_inputs_size(2);
...
base::Status SwiGLULayer::check() const {
    ...
    status = check_tensor_with_dim(input1, device_type_, data_type_, immediate_dim_);
    if (!status) {
        LOG(ERROR) << "The input tensor 1 error in the swiglu layer." << std::endl;
        return status;
    }
    status = check_tensor_with_dim(input2, device_type_, data_type_, immediate_dim_);
    if (!status) {
