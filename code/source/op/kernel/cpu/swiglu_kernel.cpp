...
namespace kernel {
void swiglu_kernel_cpu(const tensor::Tensor& input1, const tensor::Tensor& input2, const tensor::Tensor& output, void* stream) {
    UNUSED(stream);
    ...
    // SwiGLU 算子公式 SwiGLU(x1, x2) = SiLU(x1) % x2 = (x1 / (1 + e^{-x1})) % x2
    in1 %= 1.0f / (1.0f + arma::exp(-in1));
    out = in1 % in2;
}
}  // namespace kernel