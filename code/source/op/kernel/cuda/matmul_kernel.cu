...
void gemv_kernel_cu(
    ...
    gemv_kernel<256><<<gridDim, blockDim, 0, stream_>>>(in, wei, out, scale, M, N);
}
...
static __global__ __launch_bounds__(BLOCK_DIM) void fused_gemv_add_kernel(
    const float* __restrict__ in,       // [N, 1]
    const float* __restrict__ wei,      // [M, N]
    float* __restrict__ residual_add,   // [M, 1]
    int32_t M, int32_t N
) {
    constexpr int WARP_NUM = (BLOCK_DIM >> 5);
    int32_t N4 = (N >> 2);
...
void fused_gemv_add_kernel_cu(
    ...
    dim3 blockDim(512);
    cudaStream_t stream_ = stream ? static_cast<cudaStream_t>(stream) : nullptr;
    fused_gemv_add_kernel<512><<<gridDim, blockDim, 0, stream_>>>(in, wei, res_add, M, N);
}
...
void gemv_int8_kernel_cu(const tensor::Tensor& input, const tensor::Tensor& weight, const tensor::Tensor& output, const tensor::Tensor& scales, int32_t group_size, void* stream) {
    ...
    dim3 blockDim(256);
    cudaStream_t stream_ = stream ? static_cast<cudaStream_t>(stream) : nullptr;
    gemv_int8_kernel<256><<<gridDim, blockDim, 0, stream_>>>(in, wei, out, scl, group_size, M, N);
}
}  // namespace kernel