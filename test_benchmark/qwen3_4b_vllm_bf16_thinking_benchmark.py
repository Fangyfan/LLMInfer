# qwen3_4b_vllm_bf16_thinking_argmax_benchmark.py
"""
vLLM benchmark for local Qwen3-4B BF16 inference.
...
3. 本地 Qwen3-4B 模型
4. 原生 BF16 权重推理
5. 开启 Qwen3 thinking mode
6. prompt 固定为: What is AI?
7. argmax / greedy decode
8. 默认 max_tokens = 1024
9. 输出 TTFT / TPOT / 总延迟 / 吞吐
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="本地 Qwen3-4B 模型路径，例如 /path/to/Qwen3-4B 或 /path/to/qwen3-4b-instruct",
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
    ...
    print(f"Loading local Qwen3-4B model with vLLM from: {args.model}")
    print("Inference dtype: bfloat16")
    ...
    print("\n" + "=" * 70)
    print("Benchmark: Qwen3-4B vLLM BF16 Thinking Mode Argmax")
    print("=" * 70)
